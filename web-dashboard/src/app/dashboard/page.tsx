...
function ConfirmDialog({ open, onOpenChange, variant, onConfirm, isLoading }: ConfirmDialogProps) {
    ...
    const config = isRestart
        ? {
            icon: <RefreshCcw className="w-5 h-5 text-amber-700 dark:text-amber-400" />,
            ...
            confirmLabel: "確認重啟",
            loadingLabel: "正在重啟...",
            confirmClass: "bg-amber-600 hover:bg-amber-500 text-foreground",
        }
        : {
            ...
            confirmLabel: "確認關閉",
            loadingLabel: "正在關閉...",
            confirmClass: "bg-red-700 hover:bg-red-600 text-foreground",
        };
            ...
            <DialogContent className="bg-card border-border text-foreground max-w-sm" showCloseButton={false}>
                <DialogHeader>
                    <div className={`w-12 h-12 rounded-xl border flex items-center justify-center mb-2 ${isRestarted ? "bg-green-500/10 border-green-500/20" : "bg-muted border-border"}`}>
                        {isRestarted
                            ? <RefreshCcw className="w-5 h-5 text-green-700 dark:text-green-400 animate-spin" />
                ...
                {!isRestarted && (
                    <div className="rounded-lg bg-muted border border-border px-3 py-2">
                        <code className="text-xs text-cyan-700 dark:text-cyan-400 font-mono">npm start</code>
                    </div>
                )}
                {!isRestarted && (
                    <DialogFooter>
                        <Button
                            variant="outline"
                            className="w-full border-border text-muted-foreground hover:bg-muted hover:text-muted-foreground"
                            onClick={() => onOpenChange(false)}
                        >
                            關閉
                        </Button>
            ...
            <div className="h-full flex items-center justify-center p-6 bg-background">
                <div className="max-w-md w-full text-center space-y-6 animate-in fade-in zoom-in-95 duration-500">
                    <div className="inline-flex items-center justify-center w-24 h-24 bg-indigo-950/30 border border-indigo-900/50 rounded-[2rem] shadow-[0_0_40px_-10px_theme(colors.indigo.900)] mb-2">
                        <BrainCircuit className="w-12 h-12 text-indigo-700 dark:text-indigo-400" />
                    </div>
                    <div>
                    ...
                    <Link href="/dashboard/agents/create" className="inline-block w-full pt-4">
                        <Button className="w-full h-14 bg-indigo-600 hover:bg-indigo-500 text-foreground text-base font-semibold border-0 shadow-lg shadow-indigo-900/20 transition-all hover:scale-[1.02] hover:shadow-indigo-500/25">
                    ...
                    <div className="mt-6 pt-6 border-t border-border space-y-2">
                        ...
                        >
                            <div className="w-7 h-7 rounded-md bg-amber-500/10 border border-amber-500/20 flex items-center justify-center flex-shrink-0 group-hover:bg-amber-500/20 transition-colors">
                                <RefreshCcw className="w-3.5 h-3.5 text-amber-700 dark:text-amber-400" />
                            </div>
